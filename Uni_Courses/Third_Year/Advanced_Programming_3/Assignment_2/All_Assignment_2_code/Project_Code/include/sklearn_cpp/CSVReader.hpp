...
namespace CSVReader {
    ...
    /*
    This CSV file reader uses the std library to open CSV files and parse them into for-loopable lines and cells

    This is only a helper function and not a class so it has to be inline to ensure no definition errors arrise potentially, as per the assignment documentation
    It can be called using the following line:
